...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/defcoin-project/gitian.sigs.dfc.git
    git clone https://github.com/defcoin-project/defcoin-detached-sigs.git
    git clone https://github.com/devrandom/gitian-builder.git
    git clone https://github.com/defcoin-project/defcoin.git

### Defcoin maintainers/release engineers, suggestion for writing release notes
...
Ensure your gitian.sigs.dfc are up-to-date if you wish to gverify your builds against other Gitian signatures.
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in defcoin, then:

    pushd ./gitian-builder
    make -C ../defcoin/depends download SOURCES_PATH=`pwd`/cache/common
    popd
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url defcoin=/path/to/defcoin,signature=/path/to/sigs {rest of arguments}
    popd
...
### Build and sign Defcoin Core for Linux, Windows, and macOS:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit defcoin=v${VERSION} ../defcoin/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-linux --destination ../gitian.sigs.dfc/ ../defcoin/contrib/gitian-descriptors/gitian-linux.yml
    mv build/out/defcoin-*.tar.gz build/out/src/defcoin-*.tar.gz ../

    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit defcoin=v${VERSION} ../defcoin/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-unsigned --destination ../gitian.sigs.dfc/ ../defcoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit defcoin=v${VERSION} ../defcoin/contrib/gitian-descriptors/gitian-osx.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs.dfc/ ../defcoin/contrib/gitian-descriptors/gitian-osx.yml
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../defcoin/contrib/gitian-keys/README.md`.

Verify the signatures

    pushd ./gitian-builder
    ./bin/gverify -v -d ../gitian.sigs.dfc/ -r ${VERSION}-linux ../defcoin/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gverify -v -d ../gitian.sigs.dfc/ -r ${VERSION}-win-unsigned ../defcoin/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gverify -v -d ../gitian.sigs.dfc/ -r ${VERSION}-osx-unsigned ../defcoin/contrib/gitian-descriptors/gitian-osx.yml
    popd
...
- Detached signatures will then be committed to the [defcoin-detached-sigs](https://github.com/defcoin-project/defcoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed macOS binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../defcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-signed --destination ../gitian.sigs.dfc/ ../defcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gverify -v -d ../gitian.sigs.dfc/ -r ${VERSION}-osx-signed ../defcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
...
Create (and optionally verify) the signed Windows binaries:
    ...
    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../defcoin/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../defcoin/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-win-signed ../defcoin/contrib/gitian-descriptors/gitian-win-signer.yml
...
space *do not upload these to the defcoin.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the defcoin.org server.
...
- Announce the release:
  ...
  - defcoin-dev mailing list

  - blog.defcoin.org blog post

  - Update title of #defcoin and #defcoin-dev on Freenode IRC

  - Optionally twitter, reddit /r/Defcoin, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/defcoin-project/defcoin/releases/new) with a link to the archived release notes.
